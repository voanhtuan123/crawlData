...
class spider(scrapy.Spider):
    ...
    def parse_detail(self,response):
        ...
        self.categories[nhom_thuoc]['medicines'].append(data)


    def closed(self, reason):
        """Called when spider finishes - output grouped data"""
        grouped_data = []
        
        for category_name, category_data in self.categories.items():
            grouped_data.append({
                'nhom_thuoc': category_name,
                'total_medicines': len(category_data['medicines']),
                'medicines': category_data['medicines']
            })
        ...
        self.logger.info(f"Saved {len(grouped_data)} categories with total medicines")
